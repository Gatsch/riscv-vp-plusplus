...
void NetTrace::dump_transaction(bool is_read, std::string initiator, std::string target, uint64_t glob_addr,
                                unsigned char* data_ptr, unsigned int data_length, std::string delay) {
	...
	msg += initiator + ';' + target + ';' + addr.str() + ';' + delay + ';' + std::to_string(data_length) + ';' +
	       result + '\n';
